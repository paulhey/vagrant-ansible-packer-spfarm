...
Vagrant.configure(2) do |config|
  ...
  machines.each do |machine|
    ...
    raise error.new, 'machines must contain a name' if name.nil?
    ...
    config.vm.define name, primary: default, autostart: default do |cfg|
      ...
      if box
        ...
        raise error.new, 'machines must contain box or box_name and box_url'
      end
      ...
      when 'Front-End'
        ...
        config.vm.provision 'shell', path: './ansible/roles/internal/DomainController/files/SetDNS.ps1',
                                     args: '-DNS 192.168.2.19 -Network 192.168.2.16'
      ...
      when 'Database'
        ...
        # before we join the machien to the domain
        config.vm.provision 'shell', path: './ansible/roles/internal/DomainController/files/SetDNS.ps1',
                                     args: '-DNS 192.168.2.19 -Network 192.168.2.17'
      ...
      when 'Application'
        ...
        # before we join the machien to the domain
        config.vm.provision 'shell', path: './ansible/roles/internal/DomainController/files/SetDNS.ps1',
                                     args: '-DNS 192.168.2.19 -Network 192.168.2.18'
